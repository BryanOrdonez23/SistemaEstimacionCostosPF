...
return (
  <div className="container mx-auto my-8 bg-white text-gray-800 p-4 md:p-8 rounded-md shadow-md">
    <h1 className="text-blue-900 text-3xl font-bold mb-4">Lista de Proyectos</h1>
        ...
        <table className="min-w-full bg-gray-200 border border-gray-300 text-sm">
          <thead>
            <tr>
              ...
              <th className="py-2 px-2 md:px-4 border-b text-left">Categoría</th>
              <th className="py-2 px-2 md:px-4 border-b text-left">Fecha de Creación</th>
              <th className="py-2 px-2 md:px-4 border-b text-left">Última Actualización</th>
              <th className="py-2 px-2 md:px-4 border-b text-left">Acciones</th>
            </tr>
          </thead>
          <tbody>
            {proyects.map((proyect) => (
              <tr key={proyect._id} className="hover:bg-gray-300">
                <td className="py-2 px-2 md:px-4 border-b">{proyect.title}</td>
                <td className="py-2 px-2 md:px-4 border-b">{proyect.category}</td>
                <td className="py-2 px-2 md:px-4 border-b">{new Date(proyect.createdAt).toLocaleDateString()}</td>
                <td className="py-2 px-2 md:px-4 border-b">{new Date(proyect.updatedAt).toLocaleDateString()}</td>
