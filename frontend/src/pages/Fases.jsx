...
const Fase = ({ title, description, path, icon, isEnabled, iconok }) => {
  const linkClass = isEnabled
    ? "bg-blue-500 text-xs text-white px-4 py-2 rounded-full hover:bg-blue-700 transition duration-300 ease-in-out"
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow-md transition duration-300 ease-in-out transform hover:scale-105 my-4 mx-2 relative">
      {icon && <div className="absolute bottom-4 right-4">{icon}</div>}
...
const Fases = () => {
  ...
  const isFasesEnabled =
    proyect && proyect.funciones && proyect.funciones.length > 0;
  const isFasesEnabled2 =
    datosPuntosFuncion &&
    datosPuntosFuncion.functionPoints &&
    datosPuntosFuncion.functionPoints[0].calculoSA > 0;
  const isFasesEnabled3 =
    valorfactoresAjuste &&
    valorfactoresAjuste.valorFactoresAjuste &&
    valorfactoresAjuste.valorFactoresAjuste.length > 0;
  const isFasesEnabled4 =
    datosPuntosFuncion &&
    datosPuntosFuncion.functionPoints &&
    datosPuntosFuncion.functionPoints[0].calculoCA > 0;
  const isFasesEnabled5 =
    datosPuntosFuncion &&
    datosPuntosFuncion.functionPoints &&
    datosPuntosFuncion.functionPoints[0].esfuerzo > 0;
  const isFasesEnabled6 =
    datosPuntosFuncion &&
    datosPuntosFuncion.functionPoints &&
    datosPuntosFuncion.functionPoints[0].presupuesto > 0;
  const isInvolucrados =
    datosPuntosFuncion &&
    datosPuntosFuncion.functionPoints &&
    datosPuntosFuncion.functionPoints[0].involucrados &&
    datosPuntosFuncion.functionPoints[0].involucrados.length > 0;

  const isOtrosGastos =
    datosPuntosFuncion &&
    datosPuntosFuncion.functionPoints &&
    datosPuntosFuncion.functionPoints[0].otrosGastos &&
    datosPuntosFuncion.functionPoints[0].otrosGastos.length > 0;
    ...
    <div className="flex flex-col items-center justify-center  bg-CCE3FF">
      ...
      <div className="flex justify-center mt-1">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full max-w-8xl">
          ...
          <Fase
            title="Fase 2"
            description="Cálculo de puntos de función sin el ajuste (PFSA)"
            path={`/calculopfsa/${proyect._id}`}
            icon={
              <FontAwesomeIcon
                icon={faInfoCircle}
                onClick={() => mostrarPopUp("Fase2")}
                className="mr-2 text-blue-500 cursor-pointer"
                size="1x"
              />
            }
          ...
          <Fase
            title="Fase 4"
            description="Cálculo de puntos de función con el ajuste (PFCA)"
            path={`/calculopfca/${proyect._id}`}
            icon={
              <FontAwesomeIcon
                icon={faInfoCircle}
                onClick={() => mostrarPopUp("Fase4")}
                className="mr-2 text-blue-500 cursor-pointer"
                size="1x"
              />
            }
